...
field_attr       := '#offset' ':' integer_literal      // for register-block fields
                  ...
                  | '#hidden'                          // Decision #25 — encapsulation modifier
access_kind      := 'r' | 'w' | 'rw'
...
- The `#mutates` and `#cannot_mutate` clauses list **automaton names** (per Decision #3), not paths. Field-level granularity is recovered statically by the compiler from the bodies of `#mutate` statements (§2.6) and used for GA orthogonality (§7). Per Decision #9, the `#visible` / `#hidden` *visibility-clause* system from earlier drafts is removed; effect-level access is expressed by `#mutates` / `#cannot_mutate`.
- **Per-field encapsulation (Decision #25):** an `automaton_field` may carry a `#hidden` modifier (one of `field_attr`, order-independent with `#offset` / `#access`). A `#hidden` field is accessible **only** from the owning automaton's own `#transition` bodies; references from other callables (other automata's transitions, `#effect` / `#interrupt` bodies whose `#mutates` lists this automaton, `@fn` bodies) are rejected by the resolver with `E0407: HiddenFieldNotAccessible`. The semantic intent is *algebraic-trivial orthogonality*: a hidden field's basis vector cannot enter the `actual_writes` set of any callable outside the owning automaton's surface (because outside callables cannot reference it), so the §7.4 wedge product never collapses against it from outside — encapsulation by construction, with no special engine machinery. The §7.1 basis assignment treats `#hidden` fields exactly like any other field for the owning automaton's transitions; outside callables simply never contribute the bit. See `DECISIONS.md` Decision #25 for the design rationale and book Ch. 25 for the worked example.
...
6a. **Per-field encapsulation `#hidden` (Decision #25):** an `automaton_field` may carry the `#hidden` modifier (a flag — no value), in any order with `#offset` / `#access`. The parser stores the flag on the AST (`AutomatonField.hidden: bool`); duplicate `#hidden` clauses are `E0212: DuplicateClause("#hidden")`. The visibility consequence — references from outside the owning automaton's transitions emit `E0407: HiddenFieldNotAccessible` — is enforced by the resolver, not the parser; see §2.5 and `DECISIONS.md` Decision #25 for the algebraic-trivial-orthogonality reading.
...
- Every assignment inside `#mutate A { f = expr }` targets a real field `f` of automaton `A`; otherwise emit `E0303: unknown automaton field`. (Field-existence is verified upstream by `clifford-resolve` as `E0405 UnknownField`; the `E0303` code reservation here is for any future check-crate-level shape that wants to fire its own diagnostic.)
- Every **automaton** listed in a `#cannot_mutate` clause is *not* written by any `#mutate` in the body; otherwise emit `E0306: write to forbidden automaton`. (Earlier draft text wrongly said "field"; the grammar at §2.5 has always taken automaton names per Decision #3.)
